  ...
  return (
    <section>
      ...
      <div className="card-grid">
        {debtors.map((member) => (
          <button className="app-card debtor-card text-start" key={member.id} onClick={() => open(member)}>
            ...
            <div className="small text-secondary">{member.open_debts_count} prodotti · ultimo {dateTime(member.last_coppone_at)}</div>
          </button>
        ))}
      </div>
      ...
      {detail && <div className="app-card mt-3">
        ...
        {detail.items.map((debt) => <div className="list-row" key={debt.id}>
          <strong>{debt.withdrawal.product.name}</strong> · {quantity(debt.withdrawal.quantity, debt.withdrawal.product.unit)} · {money(debt.withdrawal.unit_price_cents)} cad. · residuo {money(debt.remaining_amount_cents)}
          <div className="small text-secondary">{dateTime(debt.withdrawal.withdrawn_at)} · {debt.withdrawal.notes || 'nessuna nota'}</div>
        </div>)}
